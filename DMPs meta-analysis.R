...
JHS_pheno <- read.delim("/pvol/Preprocessing/Blood/JHS/JHS Phenotypes with imputations.txt", sep = "")
...
write.table(meta_total,
            file="Meta-analysis results DMPs blood.txt",
            quote = FALSE,
            row.names = FALSE,
            col.names = TRUE,
            sep="\t")
...
#Use a dataset that has a large age range with sufficient numbers in young and old 
...
Meta_DMP_CTC <- read.delim("/pvol/EWAS/Blood/Meta-analysis/DMPs CTC/Meta-analysis results DMPs CTC blood.txt")
#upload the meta-analysis results for the DMP meta-analysis NOT adjusted for cell types 
Meta_DMP <- read.delim("/pvol/EWAS/Blood/Meta-analysis/DMPs/Meta-analysis results DMPs blood.txt")


#run correlation between the effect size for the cell type adjusted and unadjusted meta-analyses
corr <-cor.test(Meta_DMP$Effect, Meta_DMP_CTC$Effect, method = "pearson")
#0.94
corr

#Create a new table with only the CpG name, Effect size and P.value for the unadjusted analysis for plotting
ES_plot <- Meta_DMP %>% dplyr::select(CpG, Effect_size, Pvalue)
#select the same columns in the CTC meta-analysis 
ES_plot_CTC <- Meta_DMP_CTC %>% dplyr::select(CpG, Effect_size, Pvalue)

#join plots and graph
ES_plot <- ES_plot %>% left_join(ES_plot_CTC) 
...
tiff("Corrplot effect size meta-analysis with and without CTC in blood.tiff",
     ...
     unit = "in")
ggplot(data = ES_plot)+
  geom_point(aes(x = Effect_size.x, y = Effect_size.y)) +
